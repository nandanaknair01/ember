...
export default function TrackPage() {
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .insert({
        user_id: session.user.id,
        start_date: cycleLog.start_date,
        end_date: cycleLog.end_date || null,
        flow: cycleLog.flow,
        notes: cycleLog.notes
      })
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .insert({
        user_id: session.user.id,
        log_date: new Date().toISOString().split('T')[0],
        supplements: supplementLog.supplements,
        notes: null
      })
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .insert({
        user_id: session.user.id,
        log_date: new Date().toISOString().split('T')[0],
        hours: sleepLog.sleep_hours,
        quality: sleepLog.sleep_quality,
        notes: null
      })
    ...
    if (error) {
      ...
      setSleepLog({ sleep_hours: 7, sleep_quality: 3 })
      alert('Sleep log saved!')
    }
  } catch (error) {
    console.error('Error saving sleep log:', error)
